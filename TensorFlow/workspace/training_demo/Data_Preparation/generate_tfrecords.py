...
TRAIN_TFRECORD_FILE_LOC = os.getcwd() + "/training_data/" + "train.record"
EVAL_TFRECORD_FILE_LOC = os.getcwd() + "/training_data/" + "test.record"
...
def writeTfRecordFile(csvFileName, tfRecordFileName, imagesDir):
    ...
    tfRecordWriter = tf.compat.v1.python_io.TFRecordWriter(tfRecordFileName)
...
def createTfExample(singleFileData, path):
    ...
    with tf.io.gfile.GFile(os.path.join(path, '{}'.format(singleFileData.filename)), 'rb') as tensorFlowImageFile:
...
def classAsTextToClassAsInt(classAsText):
    ...
    if classAsText == 'Ball Valve':
        return 1
    if classAsText == 'Gate Valve':
        return 2
    if classAsText == 'Globe Valve':
        return 3
    if classAsText == 'Indicator':
        return 4
    if classAsText == 'Shared_indicator_01':
        return 5
    if classAsText == 'Shared_indicator_02':
        return 6
    if classAsText == 'Temperature_indicator':
        return 7
    else:
